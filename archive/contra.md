...
The 4 main presets present the project best. The Sony PVM-14L2 is the focused monitor, the JVC D-Series the firmer, cool consumer set, the Toshiba 14AF the softer household picture, and Stas's Favourite the worn RF set. The bright Mario title screen shows that the color and focus differences hold outside the dark Contra scene.

The rest of the library still overlaps too much. Several older shadow-mask presets differ mainly in softness and tint, and their coarse dot pattern can dominate at native size. The studio and Y/C group also holds near neighbors. Dying CRT has stable geometry, but its severe blur loses much of the artwork. These presets are best kept as optional looks, and the 21 should not be presented as equally convincing or as individually measured televisions.

The renders show recognizable CRT structure. Stills cannot establish phosphor motion, flicker, RF noise over time, geometry breathing or audio synchronization. The CRT photograph the user supplied also carries the camera's exposure, white balance, lens and sampling, so it is a qualitative reference and no direct color calibration target. No rendering parameter was changed to make this gallery look better.
...
| [Basement TV]({{ '/assets/images/contra-gallery/basement_tv.webp' | relative_url }}), RF | The darkest and muddiest RF picture. It works as the damaged-set extreme and makes a poor default, since face detail and highlight separation suffer. |
| [Bedroom RF 1990]({{ '/assets/images/contra-gallery/bedroom_rf_1990.webp' | relative_url }}), RF | Clearer luminance detail with soft RF color, cool highlights and the original 410-triad shadow mask. Its gain of 1.25 uses the available HDR headroom, and this SDR gallery does not reproduce its live highlight luminance. |
...
| [Late consumer aperture grille]({{ '/assets/images/contra-gallery/late_crt_wega.webp' | relative_url }}), composite | Balanced consumer sharpness and a visible grille. It looks good but overlaps the JVC D-Series and the PVM at overview size, and no WEGA was used to calibrate it. |
...
| [Reference composite]({{ '/assets/images/contra-gallery/reference_composite.webp' | relative_url }}), composite | A sharp diagnostic composite look with visible color breakup on fine patterns. It is useful as a reference and less attractive for presenting a television. |
...
| [Sony PVM-14L2]({{ '/assets/images/contra-gallery/sony_pvm_14l2.webp' | relative_url }}), composite | The strongest focused-monitor reference here, with separated scanlines, a fine vertical grille and readable white detail. It is still cleaner and more regular than the supplied camera photograph, and the match to a 14L2 is unproven. |
...
| [Stas's Favourite]({{ '/assets/images/contra-gallery/stass_favourite.webp' | relative_url }}), RF | The most distinct worn-TV main preset: softer RF color, visible convergence error and a coarse slot structure. It has rich character, but its mask is conspicuous at native size, and static captures cannot show whether its noise and load response move convincingly. |
...
| [Toshiba 14AF]({{ '/assets/images/contra-gallery/toshiba_14af43.webp' | relative_url }}), composite | A convincing softer alternative to the JVC D-Series, with wider highlights, stronger blending and a clearly visible slot mask. The grille and mask texture stay prominent in close-up. |
...
| [Warm Desktop Monitor]({{ '/assets/images/contra-gallery/warm_desktop_monitor.webp' | relative_url }}), S-Video | Warm whites and smooth Y/C detail set it apart from the neutral compact monitor. The amber tint is an intentional preference and is no evidence of a factory white balance. |
...
The review script needs NumPy and Pillow. The full images are 3840×2880 renders, and no lower-resolution image was enlarged to make them. The [frame metrics](https://github.com/yaglo/mynes/blob/master/docs/contra-gallery-metrics.json) record the phase difference, the peak light and the average light of each preset. They describe the capture and are no score of fidelity to hardware. The [23-preset audit]({{ '/archive/presets/' | relative_url }}), the [Hardware evidence note]({{ '/notes/hardware/' | relative_url }}) and the [model limits in the pipeline reference](https://github.com/yaglo/mynes/blob/master/docs/gpu-pipeline-reference.md) cover the rest.
