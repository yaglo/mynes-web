...
The images and clips on this site were written offscreen by the MyNES GPU frontend, at the size each caption gives. Captions state which values were measured. Crops are cut from the frames without resampling, and the `@1x` file offered to 1× displays is the 2×2 average of its crop. {% if facts.renderer.commit %}The renders come from renderer commit [{{ facts.renderer.commit | slice: 0, 7 }}]({{ site.project.code_repo }}/commit/{{ facts.renderer.commit }}).{% else %}The renders on the site come from several renderer commits.{% endif %}
